...
//
// Load all textures and sprites, then return a texture cache
// smoothTextures: if true, use LINEAR filtering (smoother). if false, use NEAREST (pixelated retro look)
//
export async function buildTextureCache(gl, map, thingDB, smoothTextures = true) {
  // Build a hash map of ALL textures we need
  const textureRequestMap = {}

  // Quality tier controls texture filtering:
  //   smooth = LINEAR (blurred/bilinear), retro = NEAREST (crisp pixels)
  const MAG_TEXTURES = smoothTextures ? gl.LINEAR : gl.NEAREST
  const MAG_SPRITES = gl.NEAREST // Sprites always pixelated
